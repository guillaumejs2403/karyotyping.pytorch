...
dir_name = 'BGvsCHR_epochs_'+str(n_epochs)+'_lr_'+str(lr)+'_batchSize_'+str(batch_size) + '_L' + str(n_reg) + '_' + str(L)
...
model = PSPnet.PSPNet(n_classes = 2,psp_size=512, deep_features_size=256, backend='resnet34')

# ------------------------------------
#             Load Dataset
# ------------------------------------

db_train = dl.MFISH_Dataset(root_dir = ROOT_DIR_DATASET, folder = 'train', add_augment = data_aug, rep = True, only_chr = True)
train_dataloader =  data.DataLoader(db_train, batch_size = batch_size, shuffle = True, num_workers=1)

db_val = dl.MFISH_Dataset(root_dir = ROOT_DIR_DATASET, folder = 'val', add_augment = False, rep = True, only_chr = True)
...
if decay:
    scheduler = lr_scheduler.MultiStepLR(optimizer = optimizer, milestones = schedule, gamma = 1/divider)
            ...
            if (epoch+1)%each == 0:
                ...
                for i in range(segs.shape[0]):
                    if np.random.rand() < 0.2:
                        temp_ = torch.squeeze(segs[i,:,:]).to('cpu').numpy().astype(np.uint8)*255
...
plt.close(fig)
print('Saved into',join(save_dir,dir_name))
